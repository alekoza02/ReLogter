...
class ReLogter:
    def __init__(
        ...
        self.opened_interactive_plots = []

    def _update_interactive_plots(self):
        for fig in self.opened_interactive_plots:
            self._update_single_plot(fig)

    def _update_single_plot(self, fig):
        fig.canvas.mpl_connect("close_event", self._on_close_interactive_plot)
        fig.canvas.draw_idle()
        fig.canvas.flush_events()

        fig.canvas.manager.window.after(16, self._update_single_plot, fig)

    def _on_close_interactive_plot(self, fig):
        # save stuff
        ...
    ...
    def send_report_by_email(self, config_path="credentials.ini"):
        m = MailManager()
        m.configure_mail_from_ini(config_path)

        m.send_mail(
            recipient_emails=m.recipient_emails,
            subject=f"ReLogter: {self.file_name} @ {strftime('%Y-%m-%d %H:%M:%S')}",
            body="Latest report generated by ReLogter.",
            attachment_path=f"{self.file_name}.pdf"
        )
    ...
    def write_matrix(self, array2D, transpose=False, round_precision=None):
        ...
        message = (
            ...
            + r"\["
            + r"\begin{pmatrix}"
            + r" \\ ".join([f"{' & '.join([f'{i}' if round_precision is None else f'{i:.{round_precision}f}' for i in row])}" for row in array2D])
            + r"\end{pmatrix}"
            + r"\]"
            + "\n\n"
        )
    ...
    def write_plot(
        ...
        if use_cached and len(numbers) > 0:
            ...
            if multithread:
                ...
                fig.tight_layout()
                fig.savefig(plot_path, dpi=dpi, transparent=True)
                plt.close(fig)
...
class MailManager:
    ...
    def configure_mail_from_ini(self, config_file="credentials.ini"):
        config = configparser.ConfigParser()
    
        if not os.path.exists(config_file):
            raise FileNotFoundError(f"Could not find the configuration file: {config_file}")
            
        config.read(config_file)

        self.smtp_server = config["smtp"]["server"]
        self.port = config.getint("smtp", "port") 
        
        self.sender_email = config["auth"]["sender_email"]
        self.password = config["auth"]["password"]

        self.recipient_emails = config["recipient"]["emails"].strip("[]").replace(" ", "").split(",")
    ...
    def configure_mail(self, smtp_server, port, sender_email, recipient_emails, password):
    ...
    def send_mail(self, recipient_emails, subject, body, attachment_path=None):
        ...
        for recipient in recipient_emails if isinstance(recipient_emails, list) else [recipient_emails]:
            ...
            try:
                ...
                print(f"Report successfully sent to {recipient}! Object: {subject}")
